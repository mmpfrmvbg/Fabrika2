...
def _insert_atom(conn: sqlite3.Connection, wi_id: str, *, status: str) -> str:
    unique_wi_id = f"{wi_id}_{uuid.uuid4().hex[:8]}"
    ...
    conn.execute(
        ...
        """,
        (unique_wi_id, unique_wi_id, status, now, now),
    )
    return unique_wi_id
...
def test_terminal_states_do_not_transition_to_non_terminal(tmp_path: Path, event_name: str) -> None:
    ...
    try:
        for terminal in TERMINAL_STATES:
            wi_id = _insert_atom(conn, f"wi_{terminal}", status=terminal)
...
def test_recovery_never_returns_pending_after_processing_started(tmp_path: Path, previous_status: str | None) -> None:
    ...
    try:
        wi_id = _insert_atom(conn, "wi_running", status="running")
        conn.execute(
            "UPDATE work_items SET previous_status = ? WHERE id = ?",
            (previous_status, wi_id),
        )
        conn.execute(
            """
            INSERT INTO work_item_queue (work_item_id, queue_name, priority, available_at, lease_owner, lease_until)
            VALUES (?, ?, 10, strftime('%Y-%m-%dT%H:%M:%f','now'), 'worker-1', strftime('%Y-%m-%dT%H:%M:%f','now','+30 minute'))
            """,
...
def test_concurrent_claims_have_single_lease_holder(tmp_path: Path, first_worker: str) -> None:
    ...
    try:
        wi_id = _insert_atom(conn, "wi_claim", status="ready_for_work")
        conn.execute(
            "INSERT INTO work_item_queue (work_item_id, queue_name, priority, available_at) VALUES (?, ?, 10, strftime('%Y-%m-%dT%H:%M:%f','now'))",
            (wi_id, QueueName.FORGE_INBOX.value),
        )
        conn.commit()
    finally:
    ...
    assert sum(1 for item in claims if item == wi_id) == 1
