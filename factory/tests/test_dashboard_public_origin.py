...
def test_dashboard_public_origin_uses_env_for_wildcard_bind(monkeypatch) -> None:
    ...
    assert _dashboard_public_origin(cast(BaseHTTPRequestHandler, h)) == "http://dashboard.example.com:8333"
...
def test_dashboard_public_origin_uses_default_for_wildcard_bind(monkeypatch) -> None:
    ...
    assert _dashboard_public_origin(cast(BaseHTTPRequestHandler, h)) == "http://127.0.0.1:8420"
