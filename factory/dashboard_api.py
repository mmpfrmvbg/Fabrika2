...
Опциональная авторизация POST через ``X-Internal-Token``; CORS origin из ``DASHBOARD_CORS_ORIGIN`` (по умолчанию ``http://localhost``).
"""
...
def _is_post_authorized(handler: BaseHTTPRequestHandler) -> bool:
...
class DashboardRequestHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if not _is_post_authorized(self):
            _json_response(self, {"ok": False, "error": "forbidden"}, 403)
            return
